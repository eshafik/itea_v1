...
finance_login_required = user_passes_test(lambda user: user.profile.finance, login_url='/')
...
executive_login_required = user_passes_test(lambda u: True if u.profile.is_executive else False, login_url='/')

def executive_user_required(view_func):
    decorated_view_func = login_required(executive_login_required(view_func), login_url='/')
    return decorated_view_func
